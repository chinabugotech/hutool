...
public class Issue4056Test {
	...
	public void cronTest() {
		String cron = "0 0 0 */5 * ? *";
		DateTime judgeTime = DateUtil.parse("2025-02-28 23:59:59");
	...
	void testCronAll() throws ParseException {
		ArrayList<String> cronsList = new ArrayList<>();
		ArrayList<DateTime> judgeTimes = new ArrayList<>();
		...
		// 3. 打印表头
		System.out.printf("%-28s %-28s %-26s %-26s %-6s%n",
			"Cron", "JudgeTime", "Quartz输出值", "Hutool输出值", "正确?");

		// 4. 计算并输出表格
		for (String cron : cronsList) {
			CronPattern hutoolCorn = new CronPattern(cron);
			CronExpression quartzCorn = new CronExpression(cron);
			for (DateTime judgeTime : judgeTimes) {
				Date hutoolDate = CronPatternUtil.nextDateAfter(hutoolCorn, judgeTime, true);
				Date quartzDate = quartzCorn.getNextValidTimeAfter(judgeTime);
				System.out.printf("%-28s %-28s %-28s %-28s %-6s%n",
					cron,
					DateUtil.formatDateTime(judgeTime),
					DateUtil.formatDateTime(quartzDate),
					DateUtil.formatDateTime(hutoolDate),
					quartzDate.equals(hutoolDate) ? "是" : "否"
				);
			}
		}
	}
