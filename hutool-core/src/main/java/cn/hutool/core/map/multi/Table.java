...
public interface Table<R, C, V> extends Iterable<Table.Cell<R, C, V>> {
	...
	default Map<C, V> getRow(R rowKey) {
		return Opt.ofNullable(rowMap()).map((map) -> map.get(rowKey)).orElse(null);
	}
	...
	default Set<R> rowKeySet() {
		return Opt.ofNullable(rowMap()).map(Map::keySet).orElse(null);
	}
	...
	default boolean containsColumn(C columnKey) {
		return Opt.ofNullable(columnMap()).map((map) -> map.containsKey(columnKey)).orElse(false);
	}
	...
	default Map<R, V> getColumn(C columnKey) {
		return Opt.ofNullable(columnMap()).map((map) -> map.get(columnKey)).orElse(null);
	}
	...
	default Set<C> columnKeySet() {
		return Opt.ofNullable(columnMap()).map(Map::keySet).orElse(null);
	}
	...
	default boolean containsValue(V value){
		final Collection<Map<C, V>> rows = Opt.ofNullable(rowMap()).map(Map::values).orElse(null);
		if(null != rows){
	...
	default V get(R rowKey, C columnKey) {
		return Opt.ofNullable(getRow(rowKey)).map((map) -> map.get(columnKey)).orElse(null);
	}
