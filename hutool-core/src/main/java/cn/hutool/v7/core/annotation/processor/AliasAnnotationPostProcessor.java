...
import cn.hutool.v7.core.map.ForestMap;
import cn.hutool.v7.core.map.LinkedForestMap;
import cn.hutool.v7.core.map.TreeEntry;
	...
	public void process(final SynthesizedAnnotation synthesizedAnnotation, final AnnotationSynthesizer synthesizer) {
		...
		attributeMap.forEach((attributeName, attribute) -> {
			final AnnotationAttribute resolvedAttribute = Opt.ofNullable(attributeName)
				...
				.orElse(attribute);
			Assert.isTrue(
				ObjUtil.isNull(resolvedAttribute)
