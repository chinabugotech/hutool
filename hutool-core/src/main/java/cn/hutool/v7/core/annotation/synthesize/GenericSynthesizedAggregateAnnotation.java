...
import cn.hutool.v7.core.annotation.processor.*;
import cn.hutool.v7.core.annotation.scanner.AnnotationScanner;
import cn.hutool.v7.core.annotation.scanner.MetaAnnotationScanner;
...
public class GenericSynthesizedAggregateAnnotation
	 ...
	 */
	public GenericSynthesizedAggregateAnnotation(final List<Annotation> source, final AnnotationScanner annotationScanner) {
		this(
	...
	GenericSynthesizedAggregateAnnotation(
		final Object root, final int verticalDistance, final int horizontalDistance,
		final List<Annotation> source,
		final SynthesizedAnnotationSelector annotationSelector,
		final SynthesizedAnnotationAttributeProcessor attributeProcessor,
		final Collection<SynthesizedAnnotationPostProcessor> annotationPostProcessors,
		...
		this.root = ObjUtil.defaultIfNull(root, this);
		...
		for (int i = 0; i < source.size(); i++) {
			...
			annotationScanner.scan(
				(index, annotation) -> {
					...
					final SynthesizedAnnotation newAnnotation = new MetaAnnotation(sourceAnnotation, annotation, index + 1, annotationMap.size());
					if (ObjUtil.isNull(oldAnnotation)) {
	...
	public static class MetaAnnotation extends GenericSynthesizedAnnotation<Annotation, Annotation> {
		 ...
		 * @param horizontalDistance 距离根对象的垂直距离
		 */
		protected MetaAnnotation(final Annotation root, final Annotation annotation, final int verticalDistance, final int horizontalDistance) {
