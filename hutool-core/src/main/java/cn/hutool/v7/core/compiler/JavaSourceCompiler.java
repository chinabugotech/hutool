...
public class JavaSourceCompiler {
    ...
    private JavaSourceCompiler(ClassLoader parent) {
        this.parentClassLoader = Optional.ofNullable(parent).orElseGet(ClassLoaderUtil::getClassLoader);
    }
    ...
    public ClassLoader compile(List<String> options) {
        ...
        // 创建编译器
        final JavaClassFileManager javaFileManager = new JavaClassFileManager(jcl, CompilerUtil.getFileManager());
        ...
        if (false == classPath.isEmpty()) {
            ...
            options.add("-cp");
            options.add(CollUtil.join(cp, File.pathSeparator));
        }
        ...
        try {
            ...
            IoUtil.closeIfPossible(javaFileManager);
        }
        //编译失败,收集错误信息
